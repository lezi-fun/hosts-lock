...
bool removeHostsEntries() {
    ...
    const std::string tempHostsPath = "C:\\Windows\\System32\\drivers\\etc\\hosts.tmp";
    ...
    if (!inputFile.is_open()) {
        std::cerr << "无法打开hosts文件！请以管理员权限运行此程序。" << std::endl;
        return false;
    }
    ...
    outputFile.close();
    if (foundEntries) {
        if (std::remove(hostsPath.c_str()) != 0) {
            std::cerr << "无法删除原hosts文件！" << std::endl;
            return false;
        }
        if (std::rename(tempHostsPath.c_str(), hostsPath.c_str()) != 0) {
